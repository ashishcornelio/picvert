...
class ImageConverterApp:
    def __init__(self, root):
        ...
        self.include_unknown_files = tk.BooleanVar(value=False)
    ...
    def apply_theme(self):
        ...
        if hasattr(self, "file_listbox"):
            ...
            if hasattr(self, "preview_label"):
                self.preview_label.configure(bg=c["list"], fg=c["muted"])
                self.preview_container.configure(bg=c["list"], highlightbackground="#334155")
    ...
    def build_ui(self):
        ...
        ttk.Checkbutton(
            file_frame,
            text="Include extensionless/recovered files when adding folders",
            variable=self.include_unknown_files,
            style="Dark.TCheckbutton",
        ).pack(anchor="w", pady=(6, 0))

        # 2) Preview Panel
        preview_frame = ttk.LabelFrame(right, text="2) Preview", style="Dark.TLabelframe", padding=12)
        preview_frame.pack(fill="both", expand=True, pady=(0, 10))

        c = self.colors()
        self.preview_container = tk.Frame(preview_frame, bg=c["list"], bd=0, highlightthickness=1, highlightbackground="#334155")
        self.preview_container.pack(fill="both", expand=True)

        self.preview_label = tk.Label(self.preview_container, text="Preview will appear here", bg=c["list"], fg=c["muted"], font=("Segoe UI", 10))
        self.preview_label.pack(fill="both", expand=True, padx=4, pady=4)

        settings_frame = ttk.LabelFrame(right, text="3) Conversion Settings", style="Dark.TLabelframe", padding=12)
        ...
        output_frame = ttk.LabelFrame(right, text="4) Output Folder", style="Dark.TLabelframe", padding=12)
        ...
        progress_frame = ttk.LabelFrame(right, text="5) Progress", style="Dark.TLabelframe", padding=12)
    ...
    def add_folder(self):
        ...
        if not folder:
            return
        files = []
        include_unknown = self.include_unknown_files.get()
        for name in os.listdir(folder):
            path = os.path.join(folder, name)
            if not os.path.isfile(path):
                continue
            ext = os.path.splitext(name.lower())[1]
            if ext in VALID_EXTS or (include_unknown and self.is_supported_image(path)):
                files.append(path)
    ...
    def start_conversion(self):
        ...
        threading.Thread(
            ...
            args=(output_dir, resize, width, height, quality, output_format),
            daemon=True
        ).start()

    def convert_images(self, output_dir, resize, width, height, quality, output_format):
            ...
            try:
                with Image.open(path) as img:
                    ...
                    if fmt == "AUTO":
                        ...
                        fmt = "JPEG" if source_ext in ("jpg", "jpeg", "jfif") else source_ext.upper()
                        if fmt == "JPG":
                            fmt = "JPEG"
                        if fmt == "JFIF":
                            fmt = "JPEG"
                        if fmt == "TIF":
                            fmt = "TIFF"
                        if not fmt:
                            fmt = "JPEG" if detected_format == "MPO" else detected_format
                    if fmt not in FORMAT_EXTS:
                        raise ValueError(f"Unsupported output format: {fmt or 'unknown'}")
                    ...
                    if fmt == "JPEG" and img.mode in ("RGBA", "LA", "P"):
                        ...
                        img = img.convert("RGB")
                    elif fmt in ("BMP", "ICO") and img.mode in ("RGBA", "LA", "P"):
